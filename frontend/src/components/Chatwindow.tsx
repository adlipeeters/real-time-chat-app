...
function Chatwindow() {
  ...
  useEffect(() => {
    handleEnter()
    if (liveUsersData?.liveUsersInChatroom) {
      const is_member = dataUsersOfChatroom?.getUsersOfChatroom.some((user) => user.id === userId);
      setLiveUsers(liveUsersData.liveUsersInChatroom)
      setIsUserPartOfChatroom(() => is_member)
  ...
  useEffect(() => {
    if (data?.getMessagesForChatroom) {
      const uniqueMessages = Array.from(
        new Set(data.getMessagesForChatroom.map((m) => m.id))).map((id) => data.getMessagesForChatroom.find((m) => m.id === id))
              ...
              >
                ...
                >
                  ...
                  {typingUsers.length > 0 && (
                    <Text italic c="dimmed" style={{ marginLeft: '10px' }}>
                      is typing...
                    </Text>
                  )}
                </Flex>
