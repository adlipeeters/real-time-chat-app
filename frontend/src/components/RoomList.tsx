...
interface RoomListPropsInterface {
    opened?: boolean | undefined;
    close: () => void;
    open: () => void;
}

function RoomList({ close }: RoomListPropsInterface) {
    ...
    return (
        <Flex direction={"row"} h={"100vh"} ml={"0px"}>
            {/* <Button onClick={open}>Open Drawer</Button>
            <Drawer opened={opened ? opened : false} onClose={close} title="Authentication">
            </Drawer> */}
            <Card shadow="md" p={0}>
                <Flex direction="column" align="start" style={{ padding: '10px' }}>
                        ...
                        <Flex direction={"column"}>
                            ...
                            {data?.getChatroomsForUser.map((chatroom) => (
                                <Link
                                    ...
                                    key={chatroom.id}
                                    onClick={() => { setActiveRoomId(parseInt(chatroom.id || "0")); close(); }}
                                >
                                    <Card
                                        style={
